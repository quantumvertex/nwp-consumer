...
class NWPDimensionCoordinateMap:
    ...
    def __post_init__(self) -> None:
        ...
        if self.latitude is not None:
            self.latitude = sorted(
                [np.float32(f"{lat:.4f}") for lat in self.latitude],
                reverse=True,
            )
        if self.longitude is not None:
            self.longitude = sorted([np.float32(f"{lon:.4f}") for lon in self.longitude])
